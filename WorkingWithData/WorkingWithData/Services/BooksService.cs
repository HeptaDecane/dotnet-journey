...
public class BooksService {
    ...
    public async Task<Book?> GetAsync(int id) => await _booksCollection.Find(x => x.Id == id).FirstOrDefaultAsync();

    public async Task CreateAsync(Book newBook) => await _booksCollection.InsertOneAsync(newBook);

    public async Task UpdateAsync(int id, Book updatedBook) => await _booksCollection.ReplaceOneAsync(x => x.Id == id, updatedBook);

    public async Task DeleteAsync(int id) => await _booksCollection.DeleteOneAsync(x => x.Id == id);
}